...
real SH::plegendre( int l, int m, real x )
{
  // NR original code
  int i,ll;
  real fact,pmm=1.0 ;
  ...
  ////if( m & 1 ) pmm = -pmm; // CORDON-SHORTLEY PHASE FACTOR
  if( l==m ) return pmm;
  else // Compute ~P_{m+1}^m
  {
    real pmmp1 = x*sqrt(2.0*m+3.0)*pmm;
    if( l==m+1 )  return pmmp1 ;
    else // P_l^m, l > m+1
    {
      real oldfact = sqrt( 2.0*m+3.0 ) ;
      real pll;
      for( ll=m+2; ll<=l; ll++ )
      {
